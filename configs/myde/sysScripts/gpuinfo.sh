...
generate_json() {
  ...
  for key in "${!tooltip_parts[@]}"; do
    ...
    if [[ -n "${value}" && "${value}" =~ [a-zA-Z0-9] ]]; then json+="${key}${value}"; fi
  done
...
nvidia_GPU() { #? Function to query Nvidia GPU
    ...
    if ${tired}; then 
        ...
        if [[ ${is_suspend} == *"suspend"* ]]; then
            printf '{"text":"󰤂", "tooltip":"%s ⏾ Suspended mode"}' "${primary_gpu}"; exit ;
        fi
    fi
...
nvidia_flag=${nvidia_flag:-0} intel_flag=${intel_flag:-0} 
