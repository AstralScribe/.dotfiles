#!/usr/bin/env bash
#|------------------------------------------------------|#
#|Script to configure grub, systemd-boot and pacman     |#
#|Script taken from :                                   |#
...
# exporting user_vars.sh to /etc/profile.d/
if [ ! -f /etc/profile.d/user_vars.sh ]; then
    echo -e "\033[0;32m[USER VARS]\033[0m adding global variables and path to the profile.d..."
    sudo cp user_vars.sh /etc/profile.d/user_vars.sh

else
    echo -e "\033[0;33m[SKIP]\033[0m user_vars.sh already present..."
fi
